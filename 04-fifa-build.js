...
countries.forEach(c=>{
	...
	fs.writeFileSync(__dirname + '/static/profiles/'+c.id+'.json', JSON.stringify({id:c.id,name:c.name,code:c.code,matches:matches,rankings:rankings}), 'utf8')
...
csv(fs.readFileSync(__dirname + '/fifa-teams.csv'), {columns: true}, (err, output)=>{
	...
	groupings.forEach(g=>{
		fs.writeFileSync(__dirname + '/static/matches/' + ((g.id1<g.id2)?(g.id1+'-'+g.id2):(g.id2+'-'+g.id1)) + '.json', JSON.stringify(g), 'utf8')
	})
