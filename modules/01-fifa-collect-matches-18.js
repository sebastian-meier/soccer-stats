module.exports = {
	...
	collect:(db, fs, moment, csv, request, callback)=>{
			...
			for(let key in json.competitionslist){
				...
				c.matchlist.forEach(m=>{
					...
					if(m.idHomeTeam == teams[c_team_id].id || m.idAwayTeam == teams[c_team_id].id){
						...
						let stmt = db.prepare('INSERT or IGNORE INTO teams (id, code, name) VALUES (@id, @code, @name)');
						...
						stmt = db.prepare('UPDATE matches SET scoreHome = @scoreHome, scoreAway = @scoreAway, idWinTeam = @idWinTeam, scorePenaltyAway = @scorePenaltyAway, scorePenaltyHome = @scorePenaltyHome, reasonWinCode = @reasonWinCode WHERE id = @idMatch');
						...
						match = {
						   	...
						   	scorePenaltyAway:(('scorePenaltyAway' in m))?parseInt(m.scorePenaltyAway):0, //": 5,
						   	scorePenaltyHome:(('scorePenaltyHome' in m))?parseInt(m.scorePenaltyHome):0, //": 6,
			    			reasonWinCode:('reasonWinCode' in m)?parseInt(m.reasonWinCode):0,
			    			idMatch:parseInt(m.idMatch)
			    		}
		...
		const getMatches = ()=>{
			...
			request.get('http://data.fifa.com/livescores/en/matches/byteam/'+teams[c_team_id].id+'/years/'+teams[c_team_id].years[c_year_id], {}, (error, response, body)=>{
